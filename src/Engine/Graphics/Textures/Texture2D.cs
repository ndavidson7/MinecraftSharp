...
{
    public Texture2D(GL gl, EmbeddedResource resource) : base(gl)
    {
        Bind();

        using Stream stream = resource.GetStream();
        using Image<Rgba32> image = LoadImage(stream, out Memory<Rgba32> memory);
        gl.TexImage2D(Target, 0, InternalFormat.Rgba8, (uint)image.Width, (uint)image.Height, 0, PixelFormat.Rgba, PixelType.UnsignedByte, memory.Span);
